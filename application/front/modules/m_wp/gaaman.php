...
if (isset($_POST['submit'])) {
	...
	<div class="section-top-border">
		...
		<p> <h5>Berikut ini adalah rincian hasil analisa oleh sistem untuk kamu</h5> </p><br>
		<div class="table-responsive">
			<h3>Nilai Rapor Kamu :</h3>
			<table class="table table-bordered">
				...
				<tbody>
					<?php
					$sqlnilainya = $db->query("select * from hasil where id_pengguna=$id_pengguna order by desc");
				    // $sql->data_seek(0);
				    $x = 1;
				    while ($row = $sqlnilainya->fetch_assoc()) {
				      $id_alter=$row['id_alternatif'];
				      $id_pengguna=$row['id_pengguna'];
				      $nilai=$row['nilai'];
				        $sqlprodinya = $db->query("select * from alternatif where id_alternatif=$id_alter");
				        // $sqlprodi->data_seek(0);
				        while ($rowprodi = $sqlprodinya->fetch_assoc()) {
				        $x = ++;
				          $nama_alter=$rowprodi['nama_alternatif'];

							echo "<tr>";
							echo "<td class='center'>".$nama_alter."</td>";
							echo "<td class='center'>".$nilai."</td>";
			...
			<div class="col-md-12">
		        ...
		        Highcharts.chart('container', {
		            ...
		            series:             
		             [
		                <?php 
		                $sql = $db->query("select * from hasil where id_pengguna=$id_pengguna ");
		                // $sql->data_seek(0);
		                $no = 1;
